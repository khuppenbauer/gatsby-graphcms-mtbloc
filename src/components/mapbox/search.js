...
const Mapbox = (data) => {
  ...
  const { data: geoJson, url, minCoords, maxCoords, layers, enabled, updatedMap, loaded } = data
  const geoJsonData = geoJson ? geoJson : url;
  const mapContainer = useRef(null)
  const map = useRef(null)

  console.log(['map', data, layers, enabled, updatedMap, loaded]);
  useEffect(() => {
    ...
    map.current.on('moveend', () => {
      console.log(['moveend', map.current.getBounds(), bounds]);
      dispatch({
        type: 'map', 
        data: {
  ...
  useEffect(() => {
    ...
    const bounds = new mapboxgl.LngLatBounds([minCoords.longitude, minCoords.latitude], [maxCoords.longitude, maxCoords.latitude]);
    console.log(['search', bounds, minCoords, maxCoords]);
    map.current.getSource('tracks').setData(geoJsonData);
    console.log(['bounds', map.current.getBounds(), bounds]);
